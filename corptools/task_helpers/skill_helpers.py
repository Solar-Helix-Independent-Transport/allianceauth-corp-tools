...
class SkillListCache():
    ...
    def get_and_cache_user(self, user_id):
        ...
        linked_characters = User.objects.get(id=user_id).character_ownerships.all().values_list('character__character_id', flat=True)
        skill_lists = SkillList.objects.all().order_by('order_weight','name')
        skill_list_hash = self._get_skill_list_hash(skill_lists.values_list('name'))
        ...
        if skill_lists_up_to_date:
            ...
            if cached_skills is not False: ## check if cached at all?
                cached_skills = json.loads(cached_skills)
                if cached_skills.get("doctrines", False) == skill_list_hash:  ## check what is in chache is valid
        ...
        for skill in skills:
            ...
            if char not in skill_tables:
                skill_tables[char] = {"character_id":skill.character.character.character_id, "omega": True, "skills":{}, "queue":[]}
        ...
        output_array["skills_list"] = skill_tables
