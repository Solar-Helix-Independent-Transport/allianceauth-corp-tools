...
const fmtLP = (v: number | string) => Number(v).toLocaleString("en-US", COMPACT_NUM_FORMAT);

export const Factions = ({
  data,
  isLoading = false,
}: {
  data?: components["schemas"]["GlanceFaction"];
  isLoading?: boolean;
}) => {
            ...
            <div className="d-flex h-100 align-items-center flex-wrap justify-content-center">
              {data?.lp?.evermark?.map((lp_data) => (
                <IconStatusDiv
                  ...
                  toolTipText={lp_data.corp_name ?? undefined}
                />
              ))}
            </div>
          </Card>
