...
// The {value, label} shape every plain react-select in this app uses -
// already generated by the backend as ValueLabel (it's what the
// asset/locations endpoints actually return), so components that build
// their own options list from other data reuse it too rather than each
// declaring an identical local interface.
export type SelectOption = components["schemas"]["ValueLabel"];
